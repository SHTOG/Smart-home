...
void USART2_IRQHandler(void){
	u8 Res,ReciveITEnable = 1;
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET && ReciveITEnable )  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		if(Res == 1){
			...
			data[0] = 1;
			Send_Custom_Data(USART1,0x01,1,data);//调试用，指示main函数执行完一轮
		}
		...
		// 		USART2_RX_BUF[USART2_RX_STA&0X3FFF]=Res ;//把当前接收到的数据放到USART2_RX_BUF的最后一位	
		// 		USART2_RX_STA++;//数据长度++
		// 		if(USART2_RX_BUF[1] + 2 == USART2_RX_STA) USART2_RX_STA|=0x8000;//接收完成了
		...
		// 		USART2_RX_BUF[USART2_RX_STA&0X3FFF]=Res ;//把当前接收到的数据放到USART2_RX_BUF的最后一位	
		// 		USART2_RX_STA++;//数据长度++
		// 		if(USART2_RX_BUF[1] == 'K') USART2_RX_STA|=0x8000;//接收完成了
		...
		// 		//如果第一个字节是0，则判断为刚开始接收第一字节数据
		// 		USART2_RX_BUF[1] = 0;//给数据位Zigbee命令的时候用
